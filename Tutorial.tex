\documentclass[twocolumn,nofootinbib,aps,,pra]{revtex4-1}
\usepackage[bookmarks = true, pdfpagemode = None, pdfstartview = FitH, colorlinks = true, urlcolor = blue]{hyperref}
\input{qcircuit-tikz.sty}%[braket,qm] option for released packages.
\usepackage{amsmath}  
\usepackage{listings}
\renewcommand{\arraystretch}{1.5}

% Dirac Kets % Temporary.
...
\author{Xiaodong Qi}
\affiliation{Center for Quantum Information and Control, University of New
...
{\small \begin{verbatim}\input{qcircuit-tikz}\end{verbatim}}  
%{\small \begin{verbatim}\usepackage[options]{qcircuit-tikz}\end{verbatim}}
...
%QCircuit-TikZ comes with two options - \verb=braket= and \verb=qm= - which provide defined commands for bras, kets, inner and outer products, matrix elements, and expectation values. By default, these options are not enabled, allowing you to define your own commands if you wish. 
